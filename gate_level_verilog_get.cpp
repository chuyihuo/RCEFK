...
int main(){
   ...
   int row_stg = 0;
    ...
    int num_wire = 0,num_and = 0,num_or = 0,num_not = 0; 
  ...
  for(int j = 0;j<ns;j++)
    {   
        ...
        //或门的赋值
        gate_tree[j][1].output = j;
        gate_tree[j][1].type_output = 1;
        gate_tree[j][1].use_it = 1;
        gate_tree[j][1].gate = num_or;num_or++;
       
        for(int n = 2 ;n<num_or_this+1;n++){
   ...
   for(int j = 0;j<no;j++)
    {  
        ...
        //或门的赋值
        ogate_tree[j][1].output = j;
        ogate_tree[j][1].type_output = 3;
        ogate_tree[j][1].use_it = 1;
        ogate_tree[j][1].gate = num_or;num_or++;
    ...
    for(int i = 1 ;i<no+1;i++){
        ...
        outputlist+="out"+to_string(i-1)+",";   
    }   
	outputlist.erase(outputlist.end() - 1);

    for(int i = 1 ;i<ns+1;i++){
        if(i%10 == 0){reglist +="\n";}
        reglist+="R"+to_string(i-1)+",";   
    }   
	reglist.erase(reglist.end() - 1);
    ...
    file<<"module c1126 (";
    file<<inputlist<<","<<outputlist<<");\n\n";
    file<<"input "<<inputlist<<";\n\n";
