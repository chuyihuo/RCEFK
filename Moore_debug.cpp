...
int main(){
    ...
    ifstream kiss("count_22_2.kiss",ios::in);//判断是否完成文件读取
    if(!kiss.is_open()){
   ...
   for(int j = 0;j<no;j++)
    {   numofeqn = 0;
        ...
        {   
            if(strcmp(stg_o[n][j],'1')){
              int numof_in = numofchar('-',stg_p[n]);
              if(numof_in == 0)
              {buff.push_back(stg_p[n]);
              }
              else 
              {
              expandontcare(stg_p[n],buff,numof_in);
              }      
            }
       }  
    ...
    for(int ini = 0 ;ini <simed.size();ini++){cout<<simed[ini]<<"<<"<<endl;}cout<<"==============================="<<endl;
        ...
        if (num_or_this == 0){
             for(int p = 0 ;p < ns;p++)
                {               
                         if(simed[0][p]!='-')  { if(simed[0][p] == '0'){
                                notlist += "not not" + to_string(num_not)+"(N"+to_string(num_wire)+",R"+to_string(j)+=");\n";
                                reglist += "always@(posedge clk or negedge rst)\n if(!rst)\n    O" ;
                                reglist+=to_string(j) +=" <= N" ;
                                reglist += to_string(num_wire)+=";\n";
                                reglist += "  else\n    O" + to_string(j) + " <= 1'b0;\n\n";
                                num_wire++; }
                            }else { 
                                reglist += "always@(posedge clk or negedge rst)\n if(!rst)\n    O" ;
...
cout<<">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<"<<endl;
           ...
           if(gate_tree[j][x].use_it == 1) 
            {
                ...
                case 2://notlist += "sta"+to_string(j)+" "+to_string(x)+" ";  
                    ...
                    num_not++;
                    break;
                case 4://otlist += "sta"+to_string(j)+" "+to_string(x)+" ";
                    notlist += "not not"+to_string(num_not) +"(N"+to_string(gate_tree[j][x].output)+",";
                ...
                case 5:
                    ...
                    reglist += "   else\n   R" + to_string(gate_tree[j][x].gate) + "= 1'b0;\n\n";
                    break;
                default:
                    break;
                }
    ...
    for(int j = 0 ;j<no;j++){
        for(int x = 0 ;x<andtreeleaves[j] ;x++){//cout<<"too";
           if(ogate_tree[j][x].use_it == 1) 
            {
                ...
                case 5:
                    ...
                    reglist += "  else\n       O" + to_string(ogate_tree[j][x].gate) + "=1'b0;\n";
                    break;
                default:
                    break;
                }
