...
int main(){
   ...
   for(int i = 0;i<ni;i++){
        input_vector[i] = "in"+to_string(i)+" ";//cout<<input_vector[i]<<endl;
    }   
    
    for(int i = 0 ;i<ns;i++){
        register_vector[i] = "c_state" + to_string(i) +"=";
       for(int n = 0;n<np;n++)
        {   if(strcmp(stg_n[n][i],'1')){
            ...
            for(int m = 0;m<ns;m++){
                if(strcmp(stg_p[n][m],'0')){register_vector[i].append("!n-state"+to_string(m))+" ";}
                else if(strcmp(stg_p[n][m],'1')){register_vector[i].append("n_state"+to_string(m))+" ";}
            }
    ...
    for(int i =0 ;i < no;i++){
     output_vector[i] = "out" + to_string(i)+"=";
     for(int n = 0;n<np;n++)
        {   if(strcmp(stg_n[n][i],'1')){
            
            for(int j = 0;j<ni;j++){
            if (strcmp(stg_i[n][j] ,'0')){output_vector[i].append("!"+input_vector[j]);}
            else if(strcmp(stg_i[n][j],'1')){output_vector[i].append(input_vector[j]);}
            }

            for(int m = 0;m<ns;m++){
                if(strcmp(stg_n[n][m],'0')){output_vector[i].append("!c_state"+to_string(m));}
                else if(strcmp(stg_n[n][m],'1')){output_vector[i].append("c_state"+to_string(m));}
            }
            register_vector[i].append("+");
          }
    ...
    {   numofeqn = 0;
        ...
        {   
            if(strcmp(stg_o[n][j],'1')){
              int numof_in = numofchar('-',stg_n[n])+numofchar('-',stg_i[n]);
              if(numof_in == 0)
              {buff.push_back(stg_i[n]+stg_n[n]);
              }
              else 
              {
              expandontcare(stg_i[n]+stg_n[n],buff,numof_in);
              }      
            }
       }  
